...
MODEL_NAME = "circles_basic.pth"
MODEL_PATH = Path("BinaryClassification/models")
...
# Setup a BCE (With Logits) loss function
...
for epoch in range(EPOCHS):
    ...
    model.train()
    y_logits = model(X_train).squeeze() # 1. Forward pass
    loss = loss_fn(y_logits, y_train) # 2. Calculate loss (nn.BCEWithLogits expects raw logits)
    ...
    with torch.inference_mode():
        test_logits = model(X_test).squeeze() # Forward pass
